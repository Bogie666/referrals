...
- **Vercel app** at `lexperks.com` — Express server (admin dashboard, API, /book, /share, /referral redirect, /api/cron/poll-jobs). The customer portal lives here at `/share/:code` and `/my-referrals`; the friend landing is `/referral?r=CODE`. **There is no WordPress integration anymore** — `wordpress/lex-perks.php` is a relic kept in the repo for reference only and is NOT deployed. Don't edit it; it does nothing.
...
- `wordpress/lex-perks.php` is dead code. Do not edit it. The portal is the Node `/share/:code` route at `lexperks.com`.
